...
// code with `cargo run apple` and `cargo run apples'.  Hint: use `.ends_with("s")` on the
// String reference
fn inspect(content: &String) {
    let plural_indicator = if is_plural(content) { "plural" } else { "singular" };
    println!("{} is {}", content, plural_indicator);
}

fn is_plural(content: &String) -> bool {
    let is_plural = content.ends_with("s");
    is_plural
}
